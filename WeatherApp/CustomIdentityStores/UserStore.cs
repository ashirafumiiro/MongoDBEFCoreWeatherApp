...
namespace WeatherApp.CustomIdentityStores;
...
public class UserStore<TUser> : UserStore<TUser, IdentityRole<ObjectId>, DbContext, ObjectId>
    ...
    where TRoleClaim : IdentityRoleClaim<TKey>, new()
{
    ...
    public override async Task<IList<string>> GetRolesAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        var userRoles = UserRoles.Where(p => p.UserId.Equals(user.Id)).AsEnumerable().Select(x => x.RoleId).ToList();

        var roles = await Roles.Where(p => userRoles.Contains(p.Id)).ToListAsync(cancellationToken);
    ...
    public override async Task<IList<UserLoginInfo>> GetLoginsAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        ArgumentNullException.ThrowIfNull(user);
        var userId = user.Id;
        return (await UserLogins.Where(l => l.UserId.Equals(userId)).ToListAsync(cancellationToken))
